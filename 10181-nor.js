...
function do10181(a, b, m, s, c0) {
  ...
  const a3a = NOR(!a3, NOR(b3, s0), NOR(s1, !b3));
  const a3b = NOR(NOR(!b3, a3, s2), NOR(a3, b3, s3));

  const a2a = NOR(!a2, NOR(b2, s0), NOR(s1, !b2));
  const a2b = NOR(NOR(!b2, a2, s2), NOR(a2, b2, s3));

  const a1a = NOR(!a1, NOR(b1, s0), NOR(s1, !b1));
  const a1b = NOR(NOR(!b1, a1, s2), NOR(a1, b1, s3));

  const a0a = NOR(!a0, NOR(b0, s0), NOR(s1, !b0));
  const a0b = NOR(NOR(!b0, a0, s2), NOR(a0, b0, s3));
  ...
  const n3 = m3b | m3c | m3d | m3e;
  const n2 = m2a | m2b | m2c | m2d;
  const n1 = m1a | m1b | m1c;
  const n0 = m0a | m0b;
